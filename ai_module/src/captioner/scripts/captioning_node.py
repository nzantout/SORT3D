...
class CaptioningNode:
    ...
    def __init__(
        ...
        rospy.init_node('captioning_node')

        ## Subs

        self.pose_sub = rospy.Subscriber('/state_estimation', Odometry, self.handle_pose, queue_size=1)
        self.rgb_sub = rospy.Subscriber('/camera/image', Image, self.handle_rgb, queue_size=1)
        self.sem_sub = rospy.Subscriber('/camera/semantic_image', Image, self.handle_sem, queue_size=1)
        self.query_sub = rospy.Subscriber('/object_query', String, self.handle_object_query, queue_size=1)
        if process_terrain_map_into_freespace:
            self.freespace_sub = rospy.Subscriber('/terrain_map_ext', PointCloud2, self.generate_freespace, queue_size=1)

        ## Pubs

        self.caption_pub = rospy.Publisher('/queried_captions', String, queue_size=10)
        self.freespace_pub = rospy.Publisher('/traversable_area', PointCloud2, queue_size=5)

        ## Timers

        self.map_update_timer = rospy.Timer(rospy.Duration(0.1), self.update_semantic_map)
    ...
    def update_semantic_map(self, event):
    ...
    def publish_freespace(self, freespace: np.ndarray):
        ...
        msg = ros1_bag_utils.create_point_cloud(freespace[:, :3], stamp, frame_id="map")
